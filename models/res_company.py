...
class ResCompany(models.Model):
    ...
    stm_value = fields.Monetary(
        "Monthly transportation allowance", currency_field="currency_id", readonly=False
    )
    # times
    daily_overtime = fields.Float("% Daily overtime", readonly=False, default=25.0)
    overtime_night_hours = fields.Float(
        "% Overtime night hours", readonly=False, default=75.0
    )
    hours_night_surcharge = fields.Float(
        "% Hours night surcharge", readonly=False, default=35.0
    )
    sunday_holiday_daily_overtime = fields.Float(
        "% Sunday and Holiday daily overtime", readonly=False, default=100.0
    )
    daily_surcharge_hours_sundays_holidays = fields.Float(
        "% Daily surcharge hours on sundays and holidays", readonly=False, default=75.0
    )
    sunday_night_overtime_holidays = fields.Float(
        "% Sunday night overtime and holidays", readonly=False, default=150.0
    )
    sunday_holidays_night_surcharge_hours = fields.Float(
        "% Sunday and holidays night surcharge hours", readonly=False, default=110.0
    )
    # Test
    ...
    edi_payroll_enable = fields.Boolean(
        string="Enable electronic payroll for this company",
        default=False,
        readonly=False,
    )
    # Test
    ...
    edi_payroll_always_validate = fields.Boolean(
        string="Always validate payslips", default=False
    )
    edi_payroll_enable_validate_state = fields.Boolean(
        string="Enable intermediate 'DIAN Validation' state for payroll", default=False
    )