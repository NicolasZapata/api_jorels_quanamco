...
class ResConfigSettings(models.TransientModel):
    ...
    smmlv_value = fields.Monetary(
        related="company_id.smmlv_value",
        string="SMMLV",
        readonly=False,
        currency_field="currency_id",
    )
    uvt_value = fields.Monetary(
        related="company_id.uvt_value",
        string="UVT",
        readonly=False,
        currency_field="currency_id",
    )
    stm_value = fields.Monetary(
        related="company_id.stm_value",
        string="Monthly transportation allowance",
        readonly=False,
        currency_field="currency_id",
    )
    # times
    daily_overtime = fields.Float(
        related="company_id.daily_overtime", string="% Daily", readonly=False
    )
    ...
    edi_payroll_test_set_id = fields.Char(
        related="company_id.edi_payroll_test_set_id", string="TestSetId", readonly=False
    )
    # Software ID and pin
    edi_payroll_id = fields.Char(
        related="company_id.edi_payroll_id", string="Software ID", readonly=False
    )
    edi_payroll_pin = fields.Char(
        related="company_id.edi_payroll_pin", string="Software PIN", readonly=False
    )
    ...
    def get_values(self):
        ...
        res["edi_payroll_consolidated_enable"] = (
            self.env.company.edi_payroll_consolidated_enable
        )
        res["edi_payroll_always_validate"] = (
            self.env.company.edi_payroll_always_validate
        )
